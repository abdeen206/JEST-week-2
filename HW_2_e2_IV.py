...
class Battle:
    ...
    def handle_health(self, attacker , victim, healer = None):
        ...
        def straight_fight(self, army1, army2):
            ...
            while i < max(len(army1.army_soldiers), len(army2.army_soldiers)):
                if i > len(army1.army_soldiers):
                    pass
                    # army 2 won this round
                elif i > len(army2.army_soldiers):
                    pass
                    # army 1 won this round




            if len(army1.army_soldiers) > len(army2.army_soldiers):
                print("arm1 1 wons ")
            while army1.army_soldiers[i].is_alive and army2.army_soldiers[i].is_alive:
                round_no += 1
                ...
                self.handle_health(army1.army_soldiers[i], army2.army_soldiers[i], healer)

                healer = None
                if army1.healer_exists:
                    healer = army1.healer
                self.handle_health(army2.army_soldiers[i], army1.army_soldiers[i], healer)
                ...
                if army2.army_soldiers[i].health <= 0:
                    if i == len(army2.army_soldiers) - 1:
                        print("last soldier in army 2 died -> army 1 won")
                        return True
                    else:
                        # army2.army_soldiers[j].is_alive = False
                        print("army 2 soldier number: {} has died".format(i + 1))
                        i += 1
